...
const handleSubmit = async (e) => {
  ...
  if (Object.keys(errors).every(key => errors[key] === '')) {
      try {
          await axios.put(`http://localhost:8081/note/${note_id}`, values);
          navigate('/Home');
      } catch (error) {
      ...
      <div className='bg-light rounded ps-5 pe-5 pt-3 pb-3'>
        ...
        <form onSubmit={handleSubmit}>
          <div className="mb-3">
            ...
            <input type="text" className="form-control" id="title" name='title' value={values.title} onChange={handleInput} />
            {errors.title && <span className='text-danger'>{errors.title} </span>}
          </div>
          <div className="mb-3">
            <label htmlFor="note" className="form-label fw-bold">Note</label>
            <textarea className="form-control" id="note" rows="3" name='note' value={values.note} onChange={handleInput}></textarea>
